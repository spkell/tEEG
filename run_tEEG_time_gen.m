...
Purpose:
 Script runs MVPA generalization across time.

TODO:
    1. Add back other time generalizations

    2. Why are 2nd and 3rd figures blank?

    3. What is the difference between all 4 time generalizations?
...
figure()
imagesc(data, [.3 .7]);
title(sprintf('classification accuracy for %s',conds.EEG_type{eeg_type})); % use_chan_type));
colorbar();
%nticks=5;
...
%TODO: Why are the y-axis labels backwards? which way is correct?
%Reverse y-axis labels, but keeps plot the same
%imagesc(flipud(data), [.3 .7]);

%flip plot y-axis labels
set(gca,'YDir','normal')
...
nchannel_locations_searchlight = length(ds_tl.a.fdim.values{1}); %10;
...
%}
% define searchlight neighborhood
nbrhood=cosmo_meeg_chan_neighborhood(ds_time,'count',nchannel_locations_searchlight);
...
measure_args.radius=0; %1; %default
measure_args.dimension='time';
...
%Add correct field for cosmo_map2meeg()
cdt_tf_ds.a.meeg = cdt_tf_ds.a.eeg;
cdt_tf_ds.a = rmfield(cdt_tf_ds.a,'eeg'); %remove eeg field
