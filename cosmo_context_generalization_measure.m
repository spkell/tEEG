function result=cosmo_context_generalization_measure(ds_train,ds_test,varargin)
...
% #   see the COPYING file distributed with CoSMoMVPA.           #
    ...
    check_input(ds_train,opt);
    check_input(ds_test,opt);
    

    % get training and test set
    %Splits dataset in halves based on chunks
    halves_train=split_dataset_in_train_and_test(ds_train);
    halves_test=split_dataset_in_train_and_test(ds_test);

    % split the data in two halves
    [train_values,train_splits]=split_half_by_dimension(halves_train{1},opt);
    [test_values,test_splits]=split_half_by_dimension(halves_test{2},opt);
