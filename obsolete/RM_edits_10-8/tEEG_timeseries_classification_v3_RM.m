...
% RM: I made the top-level a script for easy interface/debugging at command
% line.
%function class_avg = tEEG_timeseries_classification_v3()
    ...
    for subject=1:n_subjects
        ...
        % RM: suggeest putting above classification in a function.  then
        % compile results in a structure (e.g., g.class_raw(subject,:) =
        % sl_map.samples;) Outside of this loop, perform the group
        % averaging and plots.
        
        plot(time_values,sl_map.samples); % RM: you can do this outside this loop in one shot if the y values are in a matrix (each row different data to plot)
        hold on
        ...
        class_raw_mat(subject,:) =  sl_map.samples; % RM: same as the cell array above, but in matrix form for easier manipulation (means, ci, etc.)
    ...
    class_avg = class_avg/n_subjects; % RM: there is an easier way to do this.  in the loop above, create a NxT matrix, with N subject rows and T time point columns.  That is a compilation of all of the individual data.  Then use mean(), which will (by default) take means of each column without the need to loop over time points (the looping is done in compiled code.  in other words, mean() is "vectorized" and deals with vectors and matricies appropriately)
    
    % RM: I have stand-alone code for confidence intervals that you can
    % utilize
